...
class UsersController extends Controller
{
    public function actionIndex()
    {
        ...
        echo 'yii users/delete-not-active-users' . PHP_EOL;
    }
    ...
    public function actionDeleteNotActiveUsers()
    {
        $users = User::getNotActiveUsers();
        $this->stdout($users);
        if (!empty($users)) {
            $count = 0;
            ...
            if ($count) {
                $this->stdout('Deletet '.$count.' users.', Console::FG_GREEN, Console::BOLD);
            } else {
                $this->stderr('Something gone with deleting users!', Console::FG_RED, Console::BOLD);
            }
        } else {
            $this->stdout('Users for deleting not found!', Console::FG_GREEN, Console::BOLD);
        }
    }
